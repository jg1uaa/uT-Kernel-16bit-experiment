...
IMPORT	VP	knl_lowmem_top, knl_lowmem_limit;
	...
	if ( (UH)memend > (UH)knl_lowmem_limit ) {
		memend = knl_lowmem_limit;
	}

	/* Align top with 8 byte unit alignment */
	knl_lowmem_top = (VP)(((UH)knl_lowmem_top + 7) & ~0x0007);
	knl_imacb = (IMACB*)knl_lowmem_top;
	knl_lowmem_top = (VP)((UH)knl_lowmem_top + sizeof(IMACB));
	top = (QUEUE*)knl_lowmem_top;
	knl_imacb->memsz = (W)((UH)memend - (UH)knl_lowmem_top - sizeof(QUEUE)*2);
